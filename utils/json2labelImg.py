#!/usr/bin/python
#
# Reads labels as polygons in JSON format and converts them to label data,
# where each pixel has an ID that represents the ground truth label.
#
# Usage: json2labelImg.py [OPTIONS] <x json> <output image>
# Options:
...
from utils.helpers.annotation import Annotation
from utils.helpers.labels     import name2label

# Print the information
def printHelp():
    print('{} [OPTIONS] inputJson outputImg'.format(os.path.basename(sys.argv[0])))
    print('')
    print('Reads labels as polygons in JSON format and converts them to label data,')
...
def createLabelImage(annotation, encoding, outline=None):
    ...
    elif encoding == "categoryIds":
        background = name2label['unlabeled'].categoryId
    elif encoding == "color":
        background = name2label['unlabeled'].batch
    else:
...
def main(argv):
    ...
    if len(args) == 0:
        printError( "Missing x json file" )
    elif len(args) == 1:
