...
class Experiment:
    ...
    def model(
        ...
        if show_summary:
            ...
            summary(ApplyWrapper(perceiver, 0), input_size=(256, 256, 1), batch_size=batch_size)
            # summary(perceiver, input_size=(256, 256, 1), batch_size=batch_size)
            # summary(perceiver, input_size=(178, 218, 3), batch_size=batch_size)
            # summary(deceiver, input_size=(latent_dim,), batch_size=batch_size)
            summary(FuncApplyWrapper(deceiver, lambda _: [[2, 256, 256, 1]]), input_size=(latent_dim,), batch_size=batch_size)
        ...
        adversarial_loss = torch.nn.L1Loss(reduction='mean').to(GPU.device)
    ...
    def optimizer(models, lr, step_size, beta1=0.9, beta2=0.99, gamma=0.1):
        ...
        # optimizer_D = AdaBelief(itertools.chain(perceiver.parameters(), discriminator.parameters()), lr=lr, betas=(beta1, beta2))
...
class Training:
    ...
    def __init__(self, data, batchsize, shuffle, worker, sample_interval, n_checkpoints, logger, grad_vis, **kwargs):
        ...
        self.name = 'Impress_Perceiver_growing'
        self.logger = logger
    ...
    def train(self, models, losses, optimizers, imgs, gt):
        ...
        # gotta try masking, mask data progressively or random
        # try training autoregresively generating the data
        # !!! try progrsively growing training !!!
        ...
        self.logger.log_value_and_epoch_avg('recon_loss_train', recon_loss.item(), self.epoch, current_bs)
        self.logger.log_value_and_epoch_avg('adv_loss_train', adv_loss.item(), self.epoch, current_bs)
        ...
        real_acc = Training.accuracy(predictions, valid, current_bs)

        predictions = discriminator(z)
        fake_loss = adversarial_loss(predictions, fake)
        fake_acc = Training.accuracy(predictions, valid, current_bs)
    ...
    def __call__(self, setup: SetupStruct, epoch: int, menu):
        ...
        for i, imgs in enumerate(pbar):
            ...
            self.logger.log_value_and_epoch_avg('train_accuracy', accuracy, self.epoch, self.dataloader.batch_size)
        ...
        self.logger.log_value('train_epoch_loss_D', epoch_loss_D, Training.global_step)
        self.logger.log_value('train_epoch_accuracy', epoch_acc, Training.global_step)

        # self.logger.log_value('train_epoch_accuracy', epoch_acc, Training.global_step)
        logging.info('Training {} epoch {} Lr: {:.6f} Loss: {:.6f} D_Loss: {:.6f} D_Acc: {:.6f}'
                     .format('Impress_Deceiver_growing', epoch, scheduler.get_last_lr()[0], epoch_loss, epoch_loss_D, epoch_acc))
    ...
    async def sample_image(self, models, n_row, batches_done, epoch):
        ...
        with torch.no_grad():
            ...
            # visualize the individual generation steps step by step => make a grid for each stage
            ...
            save_image(gen_imgs.data, "{}/data/{}_random_sample_{}_{}.png".format(self.logger.log_dir, self.name, self.epoch, batches_done), nrow=n_row, normalize=True)
