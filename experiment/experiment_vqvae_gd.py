...
from models.vqvae.vqvae import GDVQVAE
...
class Experiment:
    ...
    def model(
        ...
        encoder_grid = int(2 ** (math.log(256, 2) - scale_lvl)) + (scale_kernel - 5) + (kernel-4)
        # encoder_grid = 7
        summary(vqvae.decoder, input_size=(vqvae.decoder.in_dim, encoder_grid, encoder_grid), batch_size=8)
        print(list(vqvae.named_modules()))
    ...
    def load_model(path, **kwargs):
        models = Experiment.model(**kwargs)[0]
    ...
    def setup(self) -> SetupStruct:
        ...
        models, losses = Experiment.model(**self.args['model'])
    ...
    def run(self):
        ...
        def abort():
            helper.save_checkpoint(epoch, self.logger.global_step, [setup[0]], [setup[2]], self.logger.log_dir, None,
                                   True)
            exit()
        ...
        helper.save_checkpoint(end + 1, Training.global_step, setup[0], setup[2], self.logger.log_dir,
                               self.args['training']['n_checkpoints'])
...
class Training:
    ...
    def train(self, models, losses, optimizers, imgs):
        ...
        recon_loss = mse_loss(x_hat, real_imgs)
        ...
        self.logger.log_value_and_epoch_avg('gmsd_loss', extra_recon_loss, self.epoch, self.dataloader.batch_size)
    ...
    def __call__(self, setup: SetupStruct, epoch: int, menu):
        ...
        logging.info('Training {} epoch {} Lr: {:.8f} Loss: {:.4f}'.format('Impress_VQ-VAE', epoch, scheduler_vqvae.get_last_lr()[0], epoch_loss))
    ...
    def get_vector_grid(self, vqvae):
        vector_stack = torch.zeros((vqvae.vector_quantization.n_e, 1, 256, 256))
        eg = int(2 ** (math.log(256, 2) - vqvae.scale_lvl)) + (vqvae.scale_kernel - 5)
        ...
        vector_grid = make_grid(vector_stack, nrow=vqvae.vector_quantization.n_e // 4)
        return vector_grid
    ...
    async def sample_image(self, models, n_row, batches_done, epoch):
        ...
        with torch.no_grad():
            ...
            # p_expans = 9
            # vectors = vqvae.vector_quantization.embedding.weight.repeat((p_expans, p_expans, 1, 1))
            # vectors = vectors.view(vqvae.vector_quantization.n_e, vqvae.vector_quantization.e_dim, p_expans, p_expans)
            # vectors = vqvae.decoder(vectors)
            #
            # # vqvae.vector_quantization.embedding.weight[0,:] # sollte 1x128 => repeaten auf 128x64x64 mit batch 1
            # # einzeln visualisierien
            #
            # vector_grid = make_grid(vectors, nrow=16)
            # vector_grid = helper.normalize(vector_grid)
            # save_image(vector_grid.data, "{}/data/vector_vis_{}_{}.png".format(self.logger.log_dir, epoch, batches_done), nrow=n_row)
            # fig, subplots = helper.create_image_figure(vector_grid.cpu(), 'Vector_visualization')
            # self.logger.add_figure('{}_Vector_visualization_{}_{:0>10}'.format('Impress_VQ-VAE', epoch, batches_done), fig)
