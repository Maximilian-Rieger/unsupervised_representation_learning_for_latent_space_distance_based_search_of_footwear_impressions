...
def main():
    ...
    if calculate:
        ...
        perceiver, _ = Experiment.load_model(modelWeights, batch_size=1, **configArgs['model'])
        perceiver.eval()

        # matrix = match_matrix(dataLoader, perceiver, filter_exact_match=True, dist=sim_dist)
        matrix = match_matrix(dataLoader, perceiver, filter_exact_match=True)
