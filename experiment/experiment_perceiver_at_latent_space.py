...
from models.experimental.perceiver_2 import Perceiver, Deceiver, Perceiver_Discrimator
...
class Experiment:
    ...
    def model(
        ...
        discriminator = Perceiver_Discrimator(
            depth=encoder_depth,
            num_freq_bands=num_freq_bands,
            max_freq=max_freq,
            freq_base=freq_base,
        ...
        if show_summary:
            ...
            summary(FuncApplyWrapper(deceiver, lambda _: [[2, 256, 256, input_channels]]), input_size=(latent_dim,), batch_size=batch_size)
    ...
    def optimizer(models, lr, step_size, beta1=0.9, beta2=0.99, gamma=0.1):
        ...
        optimizer = torch.optim.Adam(itertools.chain(perceiver.parameters(), deceiver.parameters()), lr=lr, betas=(beta1, beta2))
...
class Training:
    ...
    def train(self, models, losses, optimizers, imgs, gt):
        ...
        adv_loss = adversarial_loss(discriminator(latents.view(current_bs, -1)), valid)
        # embedding_loss = embedding_loss.mean()
        recon_loss = mse_loss(input_imgs, x_hat) * 100
        ...
        real_loss = adversarial_loss(discriminator(latents.view(current_bs, -1).detach()), valid)
    ...
    def get_latent_grid(model, n_rows=8):
        vector_stack = torch.zeros((model.num_latents, 1, model.latent_dim))
        for vec in range(model.latent_dim):
