...
import itertools
import numpy as np
from models.experimental.perceiver_2_vae import PerceiverVLT as Perceiver
from models.experimental.perceiver_2 import DeceiverLT as Deceiver
...
class Experiment:
    ...
    def optimizer(models, lr, step_size, beta1=0.9, beta2=0.99, gamma=0.1):
        ...
        optimizer = bnb.optim.Adam(itertools.chain(perceiver.parameters(), deceiver.parameters()), lr=lr, betas=(beta1, beta2))
    ...
    def setup(self) -> SetupStruct:
        ...
        self.training = []
        # for resolution, batchsize in [(16,96), (32,64), (64,32), (128,16)]:
        ...
        # self.validation = [
        #     Validation(**self.args['validation'], transform=self.transform_lvl_0, logger=self.logger),
        #     Validation(**self.args['validation'], transform=self.transform_lvl_1, logger=self.logger),
        #     Validation(**self.args['validation'], transform=self.transform_lvl_2, logger=self.logger),
        #     Validation(**self.args['validation'], transform=self.transform_lvl_3, logger=self.logger),
        # ]
...
class Training:
    ...
    def __init__(self, data, batchsize, shuffle, worker, sample_interval, n_checkpoints, logger, grad_vis, **kwargs):
        ...
        self.log_scale = nn.Parameter(torch.Tensor([0.0])).to(GPU.device)
    ...
    def train(self, models, losses, optimizers, imgs):
        ...
        optimizer.zero_grad()
        latents, mu, logvar = perceiver(input_imgs)

        x_hat = deceiver(latents, out_shape=img_shape)
        recon_loss = gaussian_likelihood(x_hat, self.log_scale, input_imgs)
        # recon_loss = mse_loss(input_imgs, x_hat)
        kl_div_loss = kl_divergence(latents, mu, logvar)

        # self.logger.log_value('recon_loss_train', recon_loss.mean())
        # self.logger.log_value('kl_div_loss_train', kl_div_loss.mean())

        loss = (kl_div_loss - recon_loss).mean()
        loss.backward()
