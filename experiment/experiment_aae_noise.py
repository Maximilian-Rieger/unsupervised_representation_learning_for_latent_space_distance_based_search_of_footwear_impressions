...
from models.utils import Swish, Mish
...
class Experiment:
    ...
    def model(img_shape, latent_size, losses=True, show_summary=True) -> [[nn.Module], [nn.Module]]:
        ...
        last = nn.Sigmoid
        batch_size = 6
        encoder = ConvEncoder(img_shape, latent_size=latent_size, activation=activation).to(GPU.device)
        decoder = ConvDecoder(img_shape, latent_size=latent_size, activation=activation, last=last).to(GPU.device)
        discriminator = Discriminator(latent_size=latent_size, activation=activation).to(GPU.device)
    ...
    def optimizer(models, lr, step_size, beta1=0.9, beta2=0.99, gamma=0.1):
        optimizer_G = torch.optim.Adam(itertools.chain(models[0].parameters(), models[1].parameters()), lr=lr, betas=(beta1, beta2))
        optimizer_D = torch.optim.Adam(models[2].parameters(), lr=lr, betas=(beta1, beta2))
    ...
    def load_model(path, img_shape, latent_size):
        models = Experiment.model(img_shape, latent_size)[0]
        models = helper.load_checkpoint(models, None, args={'resume': path})[0]
        return models
...
class Training:
    ...
    def __call__(self, setup, epoch, menu):
        ...
        for i, imgs in enumerate(pbar):
            ...
            noise_imgs = helper.add_noise(real_imgs.clone().detach(), amplitude=0.4)
            ...
            encoded_imgs = encoder(noise_imgs)
            ...
            if not grad_vis_g_done:
                helper.plot_grad_flow_lines(encoder.named_parameters(), "{}/data/Grad_flow_enc_{}.lines.png".format(self.logger.log_dir, epoch))
                helper.plot_grad_flow_lines(decoder.named_parameters(), "{}/data/Grad_flow_dec_{}.lines.png".format(self.logger.log_dir, epoch))
            ...
            if not grad_vis_d_done:
                helper.plot_grad_flow_lines(discriminator.named_parameters(), "{}/data/Grad_flow_dis_{}.lines.png".format(self.logger.log_dir, epoch))
                grad_vis_d_done = True
    ...
    def sample_image(self, models, batches_done, skewed=False):
        """Saves a grid of generated data"""
        encoder, decoder, discriminator = models
        # Sample noise
        batch_size = self.dataloader.batch_size
        z = Variable(Tensor(np.random.normal(0, 1, (batch_size, decoder.latent_size))).to(GPU.device))
        ...
        self.logger.add_figure('Impress_AAE_{}_Random_Sample_{:0>10}'.format(self.name, batches_done), fig)
        ...
        if skewed:
            skewed_encoding = batch.clone().detach()
            for i in range(1, batch_size):
                skewed_encoding[i] += skewed_encoding[0]
            skewed_images = decoder(skewed_encoding)
            save_image(skewed_images.data, "{}/data/skewed_sample_{}.png".format(self.logger.log_dir, batches_done), nrow=nrow_min, normalize=True)

        prediction = discriminator(batch.detach()).detach()
        prediction = prediction.repeat(1, 3 * 256 ** 2).view(batch_size, 3, 256, 256)
        batch = decoder(batch)
        batch_grid = make_grid(batch.detach(), nrow=batch_size)
        imgs = helper.add_noise(imgs.clone().detach(), amplitude=0.4)
        input_grid = make_grid(imgs, nrow=batch_size)
        prediction_grid = make_grid(prediction, nrow=batch_size)
        ...
        save_image(img_grid.data, "{}/data/Reconstruction_{}.png".format(self.logger.log_dir, batches_done), nrow=batch_size, normalize=True)
        fig, subplots = helper.create_image_figure(img_grid.cpu(), 'Reconstructed')
        self.logger.add_figure('Impress_AAE_{}_Reconstruction_{:0>10}'.format(self.name, batches_done), fig)
        plt.close()
...
class Validation:
    ...
    def sample_image(self, models, n_row, batches_done):
        ...
        encoder, decoder, discriminator = models
        # Sample noise
        # z = Variable(Tensor(np.random.normal(0, 1, (batch_size ** 2, decoder.latent_size))).to(GPU.device))
        # gen_imgs = decoder(z)

        # save_image(gen_imgs.data, "{}/data/sample_{}.png".format(self.logger.log_dir, batches_done), nrow=batch_size, normalize=True)
        # img_grid = make_grid(gen_imgs.detach(), nrow=batch_size, normalize=True)
        # fig, subplots = helper.create_image_figure(img_grid.cpu(), 'Generated', 1, 1)
        # self.logger.add_figure('{}_Sample_{:0>10}'.format('Impress_AAE', batches_done), fig)
        # fig.savefig("{}/data/Sample_fig_{:0>10}.png".format(self.logger.log_dir, batches_done))
        # plt.close()
        batch_size = self.dataloader.batch_size
        batch_start = ((batches_done * batch_size) % len(self.data))
        ...
        save_image(img_grid.data, "{}/data/Reconstruction_val_{}.png".format(self.logger.log_dir, batches_done), nrow=n_row, normalize=True)
        fig, subplots = helper.create_image_figure(img_grid.cpu(), 'Reconstructed')
        self.logger.add_figure('Impress_AAE_{}_Reconstruction_val_{:0>10}'.format(self.name, batches_done), fig)
