...
class Experiment:
    ...
    def model(img_shape, latent_size, losses=True, show_summary=True) -> [[nn.Module], [nn.Module]]:
        ...
        encoder = ConvEncoder(img_shape, latent_size=latent_size, activation=activation, activation_args=activation_args).to(GPU.device)
        decoder = ConvDecoder(img_shape, latent_size=latent_size, activation=activation, activation_args=activation_args, last=last).to(GPU.device)
        discriminator = Discriminator(latent_size=latent_size, activation=activation, activation_args=activation_args).to(GPU.device)
        ...
        # TO_DO: add loss for latent space distance ?
    ...
    def optimizer(models, lr, step_size, beta1=0.9, beta2=0.99, gamma=0.1):
        lr_g, lr_d = lr
        enc, dec, dis = models
        # optimizer_G = torch.optim.Adam(itertools.chain(enc.parameters(), dec.parameters()), lr=lr_g,
        #                                betas=(beta1, beta2))
        optimizer_GD = torch.optim.Adam(itertools.chain(enc.parameters(), dec.parameters(), dis.parameters()), lr=lr_g,
                                       betas=(beta1, beta2))
        # optimizer_D = torch.optim.Adam(itertools.chain(enc.parameters(), dis.parameters()), lr=lr_g,
        #                                betas=(beta1, beta2))
        # optimizer_D = torch.optim.Adam(dis.parameters(), lr=lr_d,
        #                                betas=(beta1, beta2))
        # why does the learning rate has to be equal between g and d !? Try a higher lr for d

        scheduler_GD = lr_scheduler.StepLR(optimizer_GD, step_size=step_size, gamma=gamma)
        ...
        return [[optimizer_GD], [scheduler_GD]]
...
class Training:
    ...
    def __call__(self, setup, epoch, menu):
        ...
        for i, imgs in enumerate(pbar):
            ...
            encoded_imgs = encoder(real_imgs.detach())
            decoded_imgs = decoder(encoded_imgs)

            encoded_imgs_clone = encoded_imgs.clone().detach()

            # Loss measures generator's ability reconstruct images
            ...
            real_loss = adversarial_loss(discriminator(encoded_imgs_clone), valid)
            real_loss = adv_loss.clone()

            # g_loss = 0.7 * adv_loss + 0.3 * rec_loss
            g_loss = 0.5 * (adv_loss + rec_loss)
            d_loss = 0.5 * (real_loss + fake_loss)

            total_loss = 0.5 * (g_loss + d_loss)
            ...
            # if not grad_vis_g_done:
            #     helper.plot_grad_flow_lines(itertools.chain(encoder.named_parameters(), decoder.named_parameters()),
            #                                 "{}/data/Grad_flow_vae_{}.lines.png".format(self.logger.log_dir, epoch))
            #     grad_vis_g_done = True
            #
            # if not grad_vis_d_done:
            #     helper.plot_grad_flow_lines(
            #         itertools.chain(encoder.named_parameters(), discriminator.named_parameters()),
            #         "{}/data/Grad_flow_encdis_{}.lines.png".format(self.logger.log_dir, epoch))
            ...
            prediction = discriminator(encoded_imgs)
            accuracy_valid = self.accuracy(prediction, valid)

            prediction = discriminator(z)
            accuracy_fake = self.accuracy(prediction, fake)
            ...
            running_loss_GD += total_loss.item() * imgs.size(0)
            ...
            self.logger.log_value('train_total_loss', total_loss.item(), Training.global_step)
        ...
        epoch_loss_GD = running_loss_GD / len(self.data)
        ...
        self.logger.log_value('train_epoch_acc', epoch_accuracy, Training.global_step)

        self.logger.log_value('train_epoch_total_loss', epoch_loss_GD, Training.global_step)
        ...
        logging.info('Training {} epoch {} Loss: {:.6f} Accuracy: {:.4f}'
                     .format('Impress_AAE', epoch, epoch_loss_GD, epoch_accuracy))
    ...
    def sample_image(self, models, batches_done, skewed_samples=False):
        ...
        gen_imgs = decoder(z)
        save_image(gen_imgs.data, "{}/data/random_sample_{}.png".format(self.logger.log_dir, batches_done), nrow=batch_size, normalize=True)
