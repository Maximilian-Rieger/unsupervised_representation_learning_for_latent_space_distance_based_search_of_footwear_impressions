...
from models.utils import Mish, Swish
...
class Experiment:
    ...
    def model(
        ...
        # tmpInput = torch.zeros([batch_size] + list(img_shape)).to(GPU.device)
        # self.logger.log_graph(encoder, tmpInput)
        # self.logger.log_graph(decoder, tmpInput)
        # self.logger.log_graph(discriminator, tmpInput)
    ...
    def setup(self) -> SetupStruct:
        ...
        self.transform = transforms.Compose([
            ...
            #         transforms.RandomResizedCrop(img_shape[1], (0.7, 1.0)),
            #         transforms.RandomAffine(45, fillcolor=255)
            #     ]),
            # ]),
            transforms.ToTensor(),
        ])
